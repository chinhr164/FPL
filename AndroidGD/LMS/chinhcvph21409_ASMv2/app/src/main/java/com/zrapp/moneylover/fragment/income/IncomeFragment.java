...
public class IncomeFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        pager = view.findViewById(R.id.vp_in);
        adapter = new IncomePager(this);
        pager.setAdapter(adapter);

        TabLayout layout = view.findViewById(R.id.tabl_in);
        new TabLayoutMediator(layout, pager, new TabLayoutMediator.TabConfigurationStrategy() {
            @Override
            public void onConfigureTab(@NonNull TabLayout.Tab tab, int position) {
                switch (position){
                    case 0: tab.setText("Khoản thu"); break;
                    case 1: tab.setText("Loại thu"); break;
                }
            }
        }).attach();
    }
